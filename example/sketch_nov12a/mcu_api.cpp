...
u8 mcu_dp_bool_update(u8 dpid,u8 value)
{
    ...
    //
    if(value == FALSE_) {
        send_len = set_wifi_uart_byte(send_len,FALSE_);
    }else {
        send_len = set_wifi_uart_byte(send_len,1);
    }
...
void wifi_uart_service(void)
{
    ...
    rx_in -= offset;
    if(rx_in > 0) {
        my_memcpy((i8 *)wifi_data_process_buf, (CONST i8 *)wifi_data_process_buf + offset, rx_in);
    }
}
...
u8 stream_trans_send(u32 id, u8 *buffer, u32 buf_len)
{
    ...
    for (i32 cnt = 0; cnt < pack_num; cnt++,map_offset += this_len) {
